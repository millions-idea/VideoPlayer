<template>
	<view class="cmd-result-page">
		<image class="cmd-result-page-image" :src="setSrc" mode="aspectFit"></image>
		<view class="cmd-result-page-text">{{setText}}</view>
		<view class="cmd-result-page-subtext">{{setSubText}}</view>
		<view v-if="defBtn || warnBtn || priBtn" class="cmd-result-page-buttons">
			<button v-if="defBtn" @tap="$_defBtn" class="button" type="default" plain="true" size="mini">{{defBtn}}</button>
			<button v-if="warnBtn" @tap="$_warnBtn" class="button" type="warn" plain="true" size="mini">{{warnBtn}}</button>
			<button v-if="priBtn" @tap="$_priBtn" class="button" type="primary" plain="true" size="mini">{{priBtn}}</button>
		</view>
	</view>
</template>
...
<style lang="less">
	...
	.cmd-result-page-image {
		width: 280upx;
		height: 280upx;
		margin-bottom: 30upx;
	}

	.cmd-result-page-text {
		line-height: 1;
		color: #c8d1e1;
		font-size: 36upx;
	}

	.cmd-result-page-subtext {
		margin-top: 16upx;
		color: #858b9c;
		font-size: 32upx;
	}

	.cmd-result-page-buttons {
		display: flex;
		flex-direction: row;

		
	}
	.cmd-result-page-buttons .button {
		margin: 10upx;
	}
</style>
