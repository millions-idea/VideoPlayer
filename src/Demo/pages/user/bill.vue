				...
				<view class="detail ">
					...
					<template v-if="state != null && state >= 0">
						...
						<template v-if="state == 0">
							<view class="item"><text class="right type trade-id">待审核</text></view>
						</template>
						<template v-if="state == 1">
							<view class="item"><text class="right type trade-id">已通过</text></view>
						</template>
						<template v-if="state == 2">
							<template v-if="remark == null">
								<view class="item"><text class="right type trade-id">被驳回</text></view>
							</template>
							<template v-else>
								<view class="item">
