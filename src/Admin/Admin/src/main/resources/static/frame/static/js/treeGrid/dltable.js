...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var temhtml='<div style="float: left;height: 28px;line-height: 28px;padding-left: '+
            ...
            +function () {//图标或占位符
                var temTreeHtml='';
                if(isTreeNode)temTreeHtml='<i class="layui-images layui-tree-head">&#xe625;</i> ';
                return temTreeHtml;
            }()
            +'</div>';
        return temhtml;
    ...
    Class.prototype.renderPage=function (count) {
        ...
        if(options.page){
            options.page = $.extend({
                ...
                ,prev: '<i class="layui-images">&#xe603;</i>'
                ,next: '<i class="layui-images">&#xe602;</i>'
...
//单元格编辑
        ...
        that.layBody.on('click', 'td div.layui-table-cell p', function(){
            ...
            if(Math.round(elemCell.prop('scrollWidth')) > Math.round(elemCell.outerWidth())){
                that.tipsIndex = layer.tips([
                    ...
                    ,elemCell.html()
                    ,'</div>'
                    ,'<i class="layui-images layui-table-tips-c">&#x1006;</i>'
                ].join(''), elemCell[0], {
                    tips: [3, '']
                    ,time: -1
                    ,anim: -1
    ...
    //自动完成渲染
    table.init();
    layui.link('/frame/static/js/treeGrid/treeGrid.css');//引入css
