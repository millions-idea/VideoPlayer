...
(function () {
    ...
    initDataTable(route + "/getWithdrawLimit", function (form, table, layer, vipTable, tableIns) {
        ...
        table.on('tool(my-data-table)', function(obj){
            ...
            }else if(layEvent == 'quick'){ //快速通过
                layer.confirm('确定后支付宝直接转账给用户，请谨慎操作！', {
                    btn: ['继续','取消'] //按钮
                }, function(){
...
function initService(r) {
    return {
         ...
         * @param callback
         */
        quickTransfer: function (param, callback) {
            param.addTime = utils.date.timestampConvert(param.addTime);
            $.post(r + "/quickTransfer", param, function (data) {
                callback(data);
            });
        },
        /**
...
function getTableColumns() {
    return [[
        ...
        , {field: 'amount', title: '金额', width: 80, templet: function (d) {
        ...
        , {fixed: 'right', field: 'remark', title: '摘要(可编辑)', edit:"text", width: 150}
        ...
        , {fixed: 'right',title: '操作', width: 240, align: 'center', templet: function (d) {
                if(d.state == 0){
                    var html = "";
                    html += " <a name=\"item-view\" class=\"layui-btn layui-btn layui-btn-sm\" lay-event=\"quick\">一键转账</a>\n";
                    html += " <a name=\"item-view\" class=\"layui-btn layui-btn layui-btn-sm\" lay-event=\"ok\">审批通过</a>\n";
                    html += "<a name=\"item-edit\" class=\"layui-btn layui-btn layui-btn-sm layui-btn-danger\" lay-event=\"pass\">拒绝</a>";
                    return html;
                }else {
                    return "-";
                }
