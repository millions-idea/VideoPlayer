...
function initService(r) {
    return {
        ...
        ,preview: function (param, callback) {
            $.get(r + "/preview", param, function (data) {
                callback(data);
            });
        }
    }
...
function getTableColumns() {
    return [[
        ...
        , {field: 'ip', title: '登录IP', width: 80}
        , {field: 'addDate', title: '注册时间', width: 80, sort: true, templet: function (d) {
        ...
        , {field: 'level', title: '级别', width: 180, sort: true, templet: function (d) {
                var levelText =  "-";
                switch (d.level){
                    case 0:
                        levelText = "普通用户";
        ...
        , {fixed: 'right',title: '操作', width: 560, align: 'center', templet: function(d){
                var html = "";
                html += '<a name="item-edit" class="layui-btn layui-btn layui-btn-xs" lay-event="preview">详情</a>';
