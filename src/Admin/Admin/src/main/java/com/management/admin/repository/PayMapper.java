...
public interface PayMapper extends  MyMapper<Pays>{
     ...
     * @return
     */
    Double selectNotWithdrawAmount(@Param("userId") Long userId, @Param("remark") String remark);
     ...
     * @return
     */
    Double selectWithdrawAmount(@Param("userId") Long userId);

    @Select("SELECT t1.*,t2.account,t3.* FROM tb_pays t1 LEFT JOIN tb_users t2 ON t1.from_uid = t2.user_id " +
    ...
    @Select("SELECT * FROM tb_pays WHERE from_uid = #{userId} OR to_uid = #{userId} ORDER BY add_time DESC LIMIT ${page},${size}")
    List<Pays> selectTransLimit(@Param("userId") Long userId, @Param("page") Integer page, @Param("size") String size);

    @Select("SELECT * FROM tb_pays WHERE from_uid = #{userId} OR to_uid = #{userId} ORDER BY add_time DESC")
    List<Pays> selectTrans(@Param("userId") Long userId);
}
