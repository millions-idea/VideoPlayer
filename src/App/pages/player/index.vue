<template>
	<view>
		<view class="container">
			<video v-if="data.videoUrl != null && data.videoUrl.length > 10 && !data.watchOne" id="myVideo" class="player"
					:poster="poster"
					:src="data.videoUrl"
					@play="onPlay"
		...
		<view class="white-container">
			...
			<view class="download">
				<image src="../../static/user/play_bg_3.png"></image>
			</view>
			<view class="blank-line-20"></view>
		</view>
		...
		<view class="white-container">
			...
			<scroll-view scroll-x="true" class="slider">
				<template v-for="(item, i) in data.adjoinList">
					<view @tap="onAdjoin(item)" class="item" :key="'slider_item_'+i" :style="{ marginRight: i === data.adjoinList.length - 1 ? '15px' : '0px'}">
	...
	export default{
		...
		methods:{
			...
			onPlay(e){
				...
				// 确认播放
				this.$api.get("api/product/player", {
			...
			getData(){
				...
				this.$api.get("api/product/getProduct", {
					productId: this.videoId
				}).then((res) => {
					uni.hideLoading();
					if(this.common.Response.isFaild(res.data)){
			...
			collect(){
				this.isCollect = true;
				this.$api.get("api/product/collect", {
					productId: this.videoId
				}).then((res) => {
			...
			unCollect(){
				this.isCollect = true;
				this.$api.get("api/product/unCollect", {
					productId: this.videoId
				}).then((res) => {
			...
			watchOne(){
				...
				this.common.window.toNew('user/order/payment', {
			...
			onAdjoin(item){
				this.common.window.toNew("player/index", {
