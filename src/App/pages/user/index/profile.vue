	...
	methods: {
		...
		formOpenWindow(name) {
			...
			if (name == 'nickName') { 
				...
				this.common.window.toNew('user/index/team',null);
			} else if (name == 'bill') {
				this.common.window.toNew('user/index/bill',null);
			}
		},
		...
		onNickNameChange(data) {
			...
			this.$api.post("api/user/editNickName",{
				nickName: data
			}).then((res) => {
				uni.hideLoading();
